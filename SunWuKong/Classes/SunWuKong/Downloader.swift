...
public class Downloader {
    ...
    public init(sessionConfiguration: URLSessionConfiguration = .default) {
        mutex = DispatchQueue(label: "com.WuKonger.mutex", attributes: .concurrent)
    ...
    public func download(_ storageRef: StorageReference?, progress: DownloadProgress? = nil, completion: @escaping DownloadCompletion) {
        sessionDelegate.delegate = self
        ...
        mutex.sync(flags: .barrier) {
            storageRef?.downloadURL(completion: { [weak self] (url, error) in
                ...
                let task: URLSessionDataTask
                if let download = weakSelf.downloads[url] {
                    task = download.task
                    download.completions.append(completion)
                } else {
                    let newTask = weakSelf.session.dataTask(with: url)
                    let download = Download(task: newTask, progress: progress, completion: completion, data: Data())
                    weakSelf.downloads[url] = download
                    task = newTask
                }
